<script>
    ...
    function toggleSelectGroup(group, txt) {
        ...
        if (txt.match(/選ぶ$/)) {
            ...
            gp.consistsOf.map((x) =>
                remove(selectedMembers, (y) => y == x.member)
            );
        }
    ...
    const toggleSelectAll = (txt) =>
        (selectedMembers = txt.match(/選ぶ$/)
            ? selectables.map((x) => x.member)
            : []);
</script>
...
<div class="memberGrouping">
    {#each selectGrouping as mbgroup, i}
        <div class="groupList">
            <StateButton
                states={mbgroup.labels}
                bind:this={gpBtn[i]}
                on:changeFrom={(ev) =>
                    toggleSelectGroup(mbgroup.gen, ev.detail.text)}
            />
